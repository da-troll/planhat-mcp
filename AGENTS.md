...
```bash
uv run pytest -q       # offline test suite; all tools, HTTP fully mocked
...
manifest.json         .mcpb bundle definition (one-click Claude Desktop install)
tests/                offline tests: every tool's method/path/body + bundle consistency
.github/workflows/    ci.yml (ruff+pytest+bundle gate), release.yml (release with .mcpb asset)
